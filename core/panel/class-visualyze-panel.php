...
if(!class_exists('Visualyze_Panel')) {
	...
	class Visualyze_Panel {
		...
		// Arguments to build page
		protected $args;
		
		// Options group name
		protected $option_group;
		
		// Info to build form structure
		public $_fields;
		
		// Table open is true, closed is false
		protected $table = false;
		
		// Tab div open is true, closed is false
		protected $tab_div;
		
		// Menu slug of current Top Level Menu
		public $top_slug;
		
		// Menu slug of current page
		public $_slug;
		
		// Array of info to help build tabs
		public $_help_tabs;
		
		// True is row, false is div
		public $_div_or_row;
		
		// Saved flag
		public $saved_flag = false;
		
		// Use google fonts, yes or no
		...
		// Hold an error flag value
		public $errors_flag = false;
		
		// Holds type of data (options, meta, tax_meta, user_meta)
		...
		public function __construct($args) {
			...
			// Determine how to handle $args param
			if(is_array($args)) {
			...
			// Add hooks for export support
			add_action('template_redirect', array($this, 'redirect_to_download'));
			...
			if(!is_array($args['menu'])) {
				...
				add_action('admin_menu', array($this, 'addTopMenuPage'));
			
			}
			
			
			// Assign page values to local vars
			$this->page_config 		= $args;
			$this->_fields			= $this->page_config['fields'];
			$this->local_images		= (isset($args['local_images'])) ? $args['div_or_row'] : false;
			...
			if(isset($args['use_with_theme'])) {
				...
				if($args['use_with_theme'] == true) {
					...
					$this->viz_path = get_stylesheet_directory_uri() . '/admin-page-class';
				
				} elseif($args['use_with_theme'] == false) {
				
					$this->viz_path = plugins_url('admin-page-class', plugin_basename(dirname(__FILE__)));
				
				} else {
				
					$this->viz_path = $args['use_with_theme'];
			...
			// Add filters and additional actions
		...
		public function menuTopLevel() {
			...
			// Setup local variable references
			$pid		= $this->args['id'];
			$title 		= $this->args['page_title'];
			$capability	= $this->args['capability'];
			$position	= $this->args['position'];
			$icon		= $this->args['icon_url'];
			
			// Create menu items
			$id 	= add_menu_page($title, $title, $capability, $pid, array($this, 'buildPage'), $icon, $position);
			$page	= add_submenu_page($id, $title, $title, $capability, $pid, array($this, 'buildPage'));
			
			if($page) {
			
				$this->_slug = $page;
				
				add_action("load-{$page}", array($this, 'loadPageAssets'));
			
			}
		
		}
		
		// Build the page in admin
		public function menuSubLevel() {
			...
			// Create the pages
			$page = add_submenu_page($this->top_slug, $title, $title, $capability, $this->createSlug(), array($this, 'buildPage'));
		...
		public function loadPageAssets() {
			
			$page = $this->_slug;
			
			// Helper tabs
			add_action("admin_head-{$page}", array($this, 'addHelpTab'));
			
			// Pluploader code
			add_action("admin_head-{$page}", array($this, 'pluploadScript'));
			
			// Scripts and styles
			add_action('admin_print_styes', array($this, 'loadDomAssets'));
			
			// Panel script
			add_action("admin_footer-{$page}", array($this, 'panelScript'));
			
			// Add missing scripts
			//add_action('admin_enqueue_scripts', array($this, 'finishAssets'));
		...
		// =====
		// WRITE PLUPLOAD FUNCTION HERE
		// =====
		
		// Returns unique slug from title and menu slug
		...
		// Add help tab, WP 3.3 and higher
		...
		// Parse help tabs to current screen
		public function addHelpTab() {
			
			$screen = get_current_screen();
			
			// Make sure we're on admin page
			if($screen->id != $this->_slug) { return; }
			
			// Help Tabs to current screen
		...
		// Change 'insert into post' button text
		public function changeInsertText($safe_text, $text) {
		...
		// Build html for our new page
		public function buildPage() {
			
			do_action('admin_page_class_before_page'); 
			
			$fid		= apply_filters('apc_form_id', 'admin_page_class', $this);
			$class		= apply_filters('apc_form_class', 'admin_page_class', $this);
			$name		= apply_filters('apc_form_name', 'admin_page_class', $this);
			
			?>
			
			<div class="wrap">
			
				<form id="vizForm" name="<?php echo $name ?>" action="" method="post" enctype="multipart/form-data">
				
					<div class="brandContainer">
						<h2 class="logo">Visualize</h2>
						<h5 class="slogan">A Wordpress Framework</h5>
					</div>
					
					<div class="utilityContainer">
						<input type="submit" class="vizbtn" name="Submit" value="Save Changes">
					</div>
					
					<?php
					
					wp_nonce_field(basename(__FILE__), 'Visualyze_Panel_nonce');
					
					if($this->saved_flag) {
						
						echo '<div class="update-status notify">';
							...
							if(is_array($this->errors) && count($this->errors) > 0) {
								...
								echo '<div class="notify success">'. __('Settings Saved', 'apc') .'</div>';
					...
					foreach($fields as $field) {
						...
						if(!in_array($field['type'], $skip)) {
							...
							if(!$this->table) {
								...
								echo '<div class="vizPanelContainer">';
						...
						if(isset($saved[$field['id']])) { $data = $saved[$field['id']]; }
						
						// Check for default values
						if(isset($field['std']) && $data == '') { $data = $field['std']; }
						
						
						// Start individual field parsing
						if(method_exists($this, 'show_field_' . $field['type'])) {
						
							echo '<div class="vizOption">';
							...
							switch($field['type']) {
								...
								case 'TABS_Listing':
									echo '<div class="vizPanelNav"><nav>';
									foreach($field['links'] as $id => $name) { echo '<a href="#'.$id.'" class="tabLink">'.$name.'</a>'; }
									echo '</nav></div><div class="vizSections">';
									break;
									
								case 'OpenTab':
									$this->tab_div = true;
									echo '<div id="'.$field['id'].'" class="settingTab">';
									do_action('admin_page_class_after_tab_open');
									break;
									
								case 'title':
									echo '<h5 class="vizOptTitle">'.$field['label'].'</h5>';
									break;
									
								case 'subtitle':
									echo '<span class="vizSubTitle">'.$field['label'].'</span>';
									break;
									
								case 'paragraph':
									echo '<p class=vizdesc>'.$field['text'].'</p>';
									break;
...
if ( ! class_exists( 'BF_Admin_Page_Class') ) :
  ...
  class BF_Admin_Page_Class {
    ...
    } // DONT FORGET TO ADD THIS FUNCTION
